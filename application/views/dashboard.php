...
<script>
  ...
  $(document).ready(function() {
    ...
    // Genera gráficos
    datagrafico(base_url_js, year)

    // Regenera gráficos si cambia el año
    $("#year").on("change", function() {
      yearselect = $(this).val()
      datagrafico(base_url_js, yearselect)
    })

    function datagrafico(base_url_js, year) {
      meses = ["Enero", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Set", "Oct", "Nov", "Dic"];
      ...
      $.ajax({
        ...
        success: function(data) {
          $.each(data, function(key, value) {
            ...
            valores[value.val_mes - 1] = valor;
          })
        }
      })

      graficar(meses, ventas, valores, year);

    }

    function graficar(meses, ventas, valores, year) {
      Highcharts.chart('grafico', {
        chart: {
          type: 'column'
        },
        title: {
          text: 'Ventas y Valores'
        },
        subtitle: {
          text: 'Año ' + year
        },
        ...
        tooltip: {
          ...
          pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: &nbsp </td>' + '<td style="padding:0"><b>$ {point.y:,.2f} </b></td></tr>',
          footerFormat: '</table>',
          shared: true,
          useHTML: true
        },
