    ...
    if (isset($_FILES["file"]["name"])) {
      ...
      foreach ($object->getWorksheetIterator() as $worksheet) {
        ...
        for ($row = 1; $row <= $highestRow; $row++) {
          $art_codigo = $worksheet->getCellByColumnAndRow(0, $row)->getValue();
          $art_nombre = $worksheet->getCellByColumnAndRow(1, $row)->getValue();
          $art_precio_compra = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
          $art_precio_venta = $worksheet->getCellByColumnAndRow(3, $row)->getCalculatedValue();
